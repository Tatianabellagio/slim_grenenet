...
rule tree_files_filling:
    input:
        pheno_file="results/arq_pi{pi}_{replicates_arq}/{heritability}/{selection}/optima{optima}/subp{replicates_sim}_minphenotype.txt",
    output:
        sim_treegen3="results/arq_pi{pi}_{replicates_arq}/{heritability}/{selection}/optima{optima}/subp{replicates_sim}_tree_output_gen3.trees",
        sim_treegen10="results/arq_pi{pi}_{replicates_arq}/{heritability}/{selection}/optima{optima}/subp{replicates_sim}_tree_output_gen10.trees",
    resources:
        mem_mb=30720,
        limit_space=1,
    conda:
...
rule tree_postprocessing:
    input:
        ...
        sim_treegen3="results/arq_pi{pi}_{replicates_arq}/{heritability}/{selection}/optima{optima}/subp{replicates_sim}_tree_output_gen3.trees",
    output:
        output_vcf=temp("results/arq_pi{pi}_{replicates_arq}/{heritability}/{selection}/optima{optima}/subp{replicates_sim}_vcf_gen3.vcf"),
    resources:
        mem_mb=30720,
        limit_space=1,
    conda:
