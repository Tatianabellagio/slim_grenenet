...
rule all:
    input:
        expand(
            "results/arq_{allele_freq}_{pi}_{beta}/{selection}_selection/optima{optima_index}/subp{replicates}_vcf_output.vcf",
        ...
        expand(
            "results/arq_{allele_freq}_{pi}_{beta}/{selection}_selection/optima{optima_index}/subp{replicates}_tree_output.trees",
            allele_freq=config['allele_freq'],
            pi=config["pi"],
            beta=config["beta"],
            selection=config["selection"],
...
rule tree_postprocessing:
    ...
    output:
        output_sim_tree_wm ="results/arq_{allele_freq}_{pi}_{beta}/{selection}_selection/optima{optima_index}/subp{replicates}_tree_output.trees",
        output_vcf ="results/arq_{allele_freq}_{pi}_{beta}/{selection}_selection/optima{optima_index}/subp{replicates}_vcf_output.vcf",
    params:
        pi=lambda wildcards: str(wildcards.pi),
        beta=lambda wildcards: str(wildcards.beta),
        allele_freq=lambda wildcards: str(wildcards.allele_freq),
